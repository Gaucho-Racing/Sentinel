...
func Subteam(args []string, s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if user.ID == "" || !(user.IsMember() || user.IsAlumni()) {
		...
		if counter == 0 {
			go service.SendDisappearingMessage(m.ChannelID, "Command usage: `!subteam <aero | business | chassis | data | drivetrain | electronics | firmware  | suspension | systems>`", 5*time.Second)
		} else {
